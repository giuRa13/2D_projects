...
namespace  ENGINE_RENDERING
{
    ...
    void BatchRenderer::Initialize()
    {
        ...
        glVertexAttribPointer(
            ...
            sizeof(Vertex),
            (void*)offsetof(Vertex, position) //offset of the first component 
            ); 
    ...
    void BatchRenderer::GenerateBatches()
    {
        ...
        int currentVertex{0}; // index of current vertex data in Vertices vector
        int currentSprite{0}; // current sprite in Sprites vector
        GLuint offset{0}; // where to start
        ...
        for(const auto& sprite : m_Sprites)
        {
            if(currentSprite == 0)
                m_Batches.emplace_back(std::make_shared<Batch>(
                    Batch{.numIndices = NUM_SPRITE_INDICES,
                    .offset = offset,
    ...
    void BatchRenderer::End()
    {
        ...
        // Sort sprites based on their layer
