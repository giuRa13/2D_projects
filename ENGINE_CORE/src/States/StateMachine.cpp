...
namespace ENGINE_CORE
{
    ...
    void StateMachine::ChangeState( const std::string& stateName, bool bRemoveState, const sol::object& enterParams )
    {
        ...
        */
        ...
        {
            ENGINE_ERROR( "Failed to change states. [{}] does not exist.", stateName );
            return;
        }
    ...
    void StateMachine::Render()
    {
        ...
        {
            ENGINE_ERROR( "Failed to render state: {}", error.what() );
        }
        catch ( ... )
        {
    ...
    void StateMachine::ExitState()
    {
        ...
        if ( stateItr == m_mapStates.end() )
        {
            ENGINE_ERROR( "Failed to exit state: {} -- State does not exist.", m_sCurrentState );
            return;
        }
