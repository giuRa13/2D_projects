...
function MoveState:OnUpdate(dt) 
	...
	local speed =  800 * dt
	local jump_pwr = 2000 * dt
	local bMoved = false

	if velocity.y < 0 then  		    -- ( going up Decrease velocity.y)
		physics:set_gravity_scale(2)
	elseif velocity.y > 0 then  		-- (down Increase velocity.y)
		physics:set_gravity_scale(10)
	end

	-- Stop the player if we are not pressing the button 
	...
	elseif Keyboard.just_released(KEY_ENTER) then
		-- todo if charged bigger projectiles
		self.m_bAttacking = fase
	end
	...
	if Keyboard.pressed(KEY_W) then
		...
		elseif self.m_NumJumps > 0 and self.m_NumJumps < 2 then 
			physics:set_gravity_scale(2)
			physics:linear_impulse(vec2(velocity.x, self.m_MaxJumpSteps * -jump_pwr * 8))
			self.m_NumJumps = 2
		end
	end
	if Keyboard.just_released(KEY_W) then 
		physics:set_linear_velocity(vec2(velocity.x, 0)) --stop double jump
	...
	if math.floor(velocity.y) == 0 and self.m_Character.m_bGrounded then 
		...
		if self.m_Character.m_bFacingLeft then 
			...
			if self.m_bAttacking then 
				sprite.start_y = 6
			else 	if math.floor(velocity.y) == 0 and self.m_Character.m_bGrounded then 
				self.m_JumpSteps = 0 
				self.m_NumJumps = 0 
				local vel = physics:get_linear_velocity()
				physics:set_linear_velocity(vec2(vel.x, 0))
